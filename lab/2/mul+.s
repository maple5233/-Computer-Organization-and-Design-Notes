...
overflow:
    andi    r23, r23,    0              # 临时变量清零
    daddi   r23, r22,    0              # 赋值为结果
    dsrl    r23, r23,   16              # 和下一步一起
    dsrl    r23, r23,   16              # 取高位
    bne     r23, r0 ,   callThePolice   # 高位不是0就报警
...
callThePolice:
    daddi   $v0,  $zero, 4              # v0 = 4
    daddi   $t0,  $zero, errorString    # t0 = errorString
    sw      $t0,  0($t5)                # 将t0塞进DATA
    sw      $v0,  0($t6)                # 将CONTROL设为4(v0)，输出 errorString 
    j       end
